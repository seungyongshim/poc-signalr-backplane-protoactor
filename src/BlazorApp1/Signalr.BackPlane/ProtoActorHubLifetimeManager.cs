...
public class ProtoActorHubLifetimeManager<THub> : HubLifetimeManager<THub> where THub : Hub
{
    ...
    public ProtoActorHubLifetimeManager(ILogger<DefaultHubLifetimeManager<THub>> logger) => _logger = logger;
    ...
    {
        if (_clientResultsManager.TryGetType(invocationId, out type)) return true;
        type = null;
        return false;
    }
}
